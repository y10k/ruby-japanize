...
部(日本語化) {
  部(:試験) {
    ...
    組(:連想配列の操作の試験, 原型: 試験単位::試験事例) {
      資料('可能なら変換'                    => [ :可能なら変換,                    :try_convert ])
      資料('ruby2互換の名前付き引数とみなす' => [ :ruby2互換の名前付き引数とみなす, :ruby2_keywords_hash ]) if (連想配列.公開された操作は定義済みか? :ruby2_keywords_hash, 偽値)
      資料('ruby2互換の名前付き引数か?'      => [ :ruby2互換の名前付き引数か?,      :ruby2_keywords_hash? ]) if (連想配列.公開された操作は定義済みか? :ruby2_keywords_hash?, 偽値)
      試験('特異操作の日本語の別名を確認') {|資料|
        日本語の別名, 元の名前 = 資料
