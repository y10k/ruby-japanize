  ...
  部(:試験) {
    組(:列挙できる物の操作の試験, 原型: 試験単位::試験事例) {
      資料('全てにおいて成り立つか?'            => [ :全てにおいて成り立つか?,            :all? ],
           ...
           '選択する'                           => [ :選択する,                           :select ],
           '抽出する'                           => [ :抽出する,                           :filter ])
      資料('抽出して対応させる'                 => [ :抽出して対応させる,                 :filter_map ]) if (列挙できる.公開された操作は定義済みか? :filter_map, 偽値)
      資料('見つけたら位置を返す'               => [ :見つけたら位置を返す,               :find_index ],
           '先頭'                               => [ :先頭,                               :first ],
           '検索'                               => [ :検索,                               :grep ],
      ...
      資料('集計'                               => [ :集計,                               :tally ]) if (列挙できる.公開された操作は定義済みか? :tally, 偽値)
      資料('連想配列に変換'                     => [ :連想配列に変換,                     :to_h ],
           '重複を削除'                         => [ :重複を削除,                         :uniq ],
           'より合わせる'                       => [ :より合わせる,                       :zip ])
    ...
    組(:列挙の操作の試験, 原型: 試験単位::試験事例) {
      資料('生産' => [ :生産, :produce ]) if (列挙.公開された操作は定義済みか? :produce, 偽値)
      試験('特異操作の日本語の別名を確認') {|資料|
        日本語の別名, 元の名前 = 資料
        別名であることを確認(列挙, 日本語の別名, 元の名前)
        含まれることを確認(列挙.公開された操作の一覧, 日本語の別名)
      }
    ...
    組(:列挙_継手の操作の試験, 原型: 試験単位::試験事例) {
      資料('個別に'   => [ :個別に,   :each ],
           '巻き戻す' => [ :巻き戻す, :rewind ],
           '大きさ'   => [ :大きさ,   :size ])
      試験('公開された操作の日本語の別名を確認') {|資料|
        日本語の別名, 元の名前 = 資料
        ある継手 = (1..2).個別に.接続((3..4).個別に)
        生成物であることを確認(列挙::継手, ある継手)
        別名であることを確認(ある継手, 日本語の別名, 元の名前)
        含まれることを確認(列挙::継手.生成物が持つ公開された操作の一覧, 日本語の別名)
      }
    }

    組(:列挙_生成器の操作の試験, 原型: 試験単位::試験事例) {
      資料('個別に'   => [ :個別に, :each ])
      試験('公開された操作の日本語の別名を確認') {|資料|
        日本語の別名, 元の名前 = 資料
        ある生成器 = 列挙::生成器.新規{}
        生成物であることを確認(列挙::生成器, ある生成器)
        別名であることを確認(ある生成器, 日本語の別名, 元の名前)
        含まれることを確認(列挙::生成器.生成物が持つ公開された操作の一覧, 日本語の別名)
      }
    }

    組(:列挙_補給器の操作の試験, 原型: 試験単位::試験事例) {
      資料('供給'   => [ :供給, :yield ])
      試験('公開された操作の日本語の別名を確認') {|資料|
        日本語の別名, 元の名前 = 資料
        ある補給器 = 列挙::補給器.新規{}
        生成物であることを確認(列挙::補給器, ある補給器)
        別名であることを確認(ある補給器, 日本語の別名, 元の名前)
        含まれることを確認(列挙::補給器.生成物が持つ公開された操作の一覧, 日本語の別名)
      }
    }

    if (defined? Enumerator::Producer) then
      組(:列挙_生産器の操作の試験, 原型: 試験単位::試験事例) {
        資料('個別に'   => [ :個別に, :each ])
        試験('公開された操作の日本語の別名を確認') {|資料|
          日本語の別名, 元の名前 = 資料
          ある生産器 = 列挙::生産器.新規
          生成物であることを確認(列挙::生産器, ある生産器)
          別名であることを確認(ある生産器, 日本語の別名, 元の名前)
          含まれることを確認(列挙::生産器.生成物が持つ公開された操作の一覧, 日本語の別名)
        }
      }
    end
    ...
    組(:列挙_遅延評価の操作の試験, 原型: 試験単位::試験事例) {
      資料('連続した塊に分ける'                 => [ :連続した塊に分ける,                 :chunk ],
           ...
           '捨てる'                             => [ :捨てる,                             :drop ],
           'この条件で捨てる'                   => [ :この条件で捨てる,                   :drop_while ])
      資料('所望'                               => [ :所望,                               :eager ]) if (列挙::遅延評価.公開された操作は定義済みか? :eager, 偽値)
      資料('列挙に変換'                         => [ :列挙に変換,                         :to_enum ],
           '列挙する'                           => [ :列挙する,                           :enum_for ],
           '選択する'                           => [ :選択する,                           :select ],
           '全て見つける'                       => [ :全て見つける,                       :find_all ],
           '抽出する'                           => [ :抽出する,                           :filter ])
      資料('抽出して対応させる'                 => [ :抽出して対応させる,                 :filter_map ]) if (列挙::遅延評価.公開された操作は定義済みか? :filter_map, 偽値)
      資料('直ちに評価する'                     => [ :直ちに評価する,                     :force ],
           '検索'                               => [ :検索,                               :grep ],
           '対象外を検索'                       => [ :対象外を検索,                       :grep_v ],
           '遅延評価'                           => [ :遅延評価,                           :lazy ],
           '除く'                               => [ :除く,                               :reject ],
