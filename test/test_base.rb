  ...
  部(:試験) {
    組(:部の操作の試験, 原型: 試験単位::試験事例) {
      ...
      資料('別名'                             => [ :別名,                             :alias_method ],
           ...
           '定数は定義済みか?'                => [ :定数は定義済みか?,                :const_defined? ],
           # '不明な定数'                     => [ :不明な定数,                       :const_missing ], # 再定義済みのため別名ではない
           '定数を取得'                       => [ :定数を取得,                       :const_get ],
           '定数を設定'                       => [ :定数を設定,                       :const_set ])
      資料('定数が定義された場所'             => [ :定数が定義された場所,             :const_source_location ]) if (部.公開された操作は定義済みか? :const_source_location, 偽値)
      資料('定数の一覧'                       => [ :定数の一覧,                       :constants ],
           '定義'                             => [ :定義,                             :define_method ],
      ...
      資料('機能を付加する'           => [ :機能を付加する,          :append_features ],
           ...
           # '拡張した時'             => [ :拡張した時,              :extended  ],        # 再定義済みのため別名ではない
           # '付加された時'           => [ :付加された時,            :included ],         # 再定義済みのため別名ではない
           # '操作が追加された時'     => [ :操作が追加された時,      :method_added ],     # 再定義済みのため別名ではない
           # '操作が除去された時'     => [ :操作が除去された時,      :method_removed ],   # 再定義済みのため別名ではない
           # '操作が未定義にされた時' => [ :操作が未定義にされた時,  :method_undefined ], # 再定義済みのため別名ではない
           '部の関数'                 => [ :部の関数,                :module_function ],
           '機能を付与する'           => [ :機能を付与する,          :prepend_features ],
           # '付与された時'           => [ :付与された時,            :prepended ],        # 再定義済みのため別名ではない
           '秘密'                     => [ :秘密,                    :private ],
           '保護'                     => [ :保護,                    :protected ],
           '公開'                     => [ :公開,                    :public ],
           '限定拡張'                 => [ :限定拡張,                :refine ],
           '定数を除去する'           => [ :定数を除去する,          :remove_const ])
      資料('ruby2互換の名前付き引数'  => [ :ruby2互換の名前付き引数, :ruby2_keywords ]) if (部.秘密の操作は定義済みか? :ruby2_keywords, 偽値)
      資料('限定拡張を使用'           => [ :限定拡張を使用,          :using ])
    ...
    組(:各種操作を日本語化できることの試験, 原型: 試験単位::試験事例) {
      ...
      試験('入れ物の操作を日本語化できることを確認') {|資料|
        返り値, 入れ物の操作の引数, 入れ物の操作の名前付きの引数, 日本語の入れ物の操作, 本来の入れ物の操作, 別名の一覧 = 資料
        別名の一覧 ||= {}
      ...
      資料('分解'       => [ [], [],      :分解,       :deconstruct ],
           'キーで分解' => [ {}, [ nil ], :キーで分解, :deconstruct_keys ])
      試験('物を分解する操作を日本語化できることを確認') {|資料|
        分解した結果, 分解操作の引数, 日本語の分解操作, 本来の分解操作 = 資料

        分解操作を持たない物 = 組.新規
        組(分解操作を持たない物) {
          # 操作定義の変更の影響を受けないようにするため、上位の組で未定義にする。
          if (操作は定義済みか? 日本語の分解操作) then
            操作を未定義にする 日本語の分解操作
          end
          if (操作は定義済みか? 本来の分解操作) then
            操作を未定義にする 本来の分解操作
          end
        }
        ...
        ある組 = 組.新規(分解操作を持たない物)
        ある物 = ある組.新規
        例外が発生することを確認(操作が無いため失敗) { ある物.公開された範囲で送信(日本語の分解操作, *分解操作の引数) }
        例外が発生することを確認(操作が無いため失敗) { ある物.公開された範囲で送信(本来の分解操作, *分解操作の引数) }
        ...
        等しいことを確認(分解した結果, ある物.公開された範囲で送信(日本語の分解操作))
        等しいことを確認(分解した結果, ある物.公開された範囲で送信(本来の分解操作))
        ...
        等しいことを確認(分解した結果, ある物.公開された範囲で送信(日本語の分解操作))
        等しいことを確認(分解した結果, ある物.公開された範囲で送信(本来の分解操作))
        別名であることを確認(ある物, 日本語の分解操作, 本来の分解操作, '別名を上書きしないことを確認')
        ...
        等しいことを確認(分解した結果, ある物.公開された範囲で送信(日本語の分解操作, *分解操作の引数))
        等しいことを確認(分解した結果, ある物.公開された範囲で送信(本来の分解操作, *分解操作の引数))

        ある物.特異組{
          操作を除去する 日本語の分解操作
        }
        例外が発生することを確認(操作が無いため失敗) { ある物.公開された範囲で送信(日本語の分解操作) }
        例外が発生することを確認(操作が無いため失敗) { ある物.公開された範囲で送信(本来の分解操作) }
