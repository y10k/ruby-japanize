  ...
  部(:試験) {
    組(:制御命令の試験, 原型: 試験単位::試験事例) {
      ...
      試験('物の手続きを渡す命令を確認') {|資料|
        ...
        回数 = 0
        評価結果 = \
        ある物.手続きを渡す(:ある操作, *呼び出しの引数, **呼び出しの名前付き引数) {|制御, 数|
          呼び出し履歴 << [ :手続き, 数 ]
          回数 += 1
        }
      ...
      資料('引数無し,名前付き引数無し' => [ [],       {} ],
           ...
           '引数無し,名前付き引数有り' => [ [],       { イ: 'ロ' } ],
           '引数有り,名前付き引数有り' => [ [ 'イ'],  { ロ: 'ハ' } ])
      試験('物の手続きを渡す命令が秘密の操作に失敗することを確認') {|資料|
        呼び出しの引数, 呼び出しの名前付き引数 = 資料
        呼び出し履歴 = []
      ...
      資料('引数無し,名前付き引数無し' => [ [],       {} ],
           ...
           '引数無し,名前付き引数有り' => [ [],       { イ: 'ロ' } ],
           '引数有り,名前付き引数有り' => [ [ 'イ'],  { ロ: 'ハ' } ])
      試験('物の手続きを渡す命令が秘密の操作を呼び出せることを確認') {|資料|
        呼び出しの引数, 呼び出しの名前付き引数 = 資料
        呼び出し履歴 = []
        ...
        回数 = 0
        評価結果 = \
        ある物.手続きを渡す(:ある操作, *呼び出しの引数, 秘密の操作を許可: 真値, **呼び出しの名前付き引数) {|制御, 数|
          呼び出し履歴 << [ :手続き, 数 ]
          回数 += 1
        }
      ...
      試験('複数の引数を伴う繰り返し命令を確認') {
        実行履歴 = []

        評価結果 = \
        繰り返す(('a'..'z').each_with_index) {|制御, 項目, 順番|
          実行履歴 << [ 項目, 順番 ]
        }

        等しいことを確認(配列(('a'..'z').zip(0..25)), 実行履歴)
        等しいことを確認('a'..'z', 評価結果)
      }
