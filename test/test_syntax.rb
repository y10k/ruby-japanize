...
部(日本語化) {
  部(:試験) {
    組(:制御命令の試験, 原型: 試験単位::試験事例) {
      ...
      試験('領域の折り返し処理を確認') {|資料|
        発生させる例外の型, 発生させる例外の伝文, 期待する実行履歴, 期待する返り値 = 資料
        ...
        領域{|制御|
          ...
          制御.例外を補足(入出力の失敗) {|発生した例外|
            実行履歴 << :入出力の失敗
            一種であることを確認(入出力の失敗, 発生した例外)
            生成物であることを確認(ファイル終端のため失敗, 発生した例外)
            等しいことを確認(発生させる例外の伝文, 文字列(発生した例外))
            :乙
          }
          制御.例外を補足{|発生した例外|
            実行履歴 << :標準的な失敗
            一種であることを確認(標準的な失敗, 発生した例外)
            生成物であることを確認(浮動小数点の定義域外のため失敗, 発生した例外)
            等しいことを確認(発生させる例外の伝文, 文字列(発生した例外))
            if (再実行するか) then
              再実行するか = 偽値
              制御.やり直す
