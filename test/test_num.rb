...
部(日本語化) {
  部(:試験) {
    ...
    組(:疑似乱数生成器_書式の操作の試験, 原型: 試験単位::試験事例) {
      資料('乱数'       => [ :乱数,       :rand ],
           '乱数を取得' => [ :乱数を取得, :random_number ])
      試験('公開された操作の日本語の別名を確認') {|資料|
        日本語の別名, 元の名前 = 資料
        ある乱数発生器 = 疑似乱数生成器.新規
        一種であることを確認(疑似乱数生成器::書式, ある乱数発生器)
        別名であることを確認(ある乱数発生器, 日本語の別名, 元の名前)
        含まれることを確認(疑似乱数生成器::書式.生成物が持つ公開された操作の一覧, 日本語の別名)
      }
    }
