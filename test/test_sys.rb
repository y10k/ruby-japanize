  ...
  部(:試験) {
    組(:束縛の操作の試験, 原型: 試験単位::試験事例) {
      資料('評価'                  => [ :評価,                  :eval ],
           ...
           '受信先'                => [ :受信先,                :receiver ],
           '定義された場所'        => [ :定義された場所,        :source_location ])
    ...
    組(:手続き型の操作の試験, 原型: 試験単位::試験事例) {
      ...
      資料('ruby2互換の名前付き引数' => [ :ruby2互換の名前付き引数, :ruby2_keywords ]) if (手続き型.公開された操作は定義済みか? :ruby2_keywords, 偽値)
      資料('定義された場所'          => [ :定義された場所,          :source_location ])
      試験('公開された操作の日本語の別名を確認') {|資料|
        日本語の別名, 元の名前 = 資料
    ...
    組(:未束縛の操作の操作の試験, 原型: 試験単位::試験事例) {
      資料('項数'           => [ :項数,           :arity ],
           '束縛'           => [ :束縛,           :bind ])
      資料('束縛して呼ぶ'   => [ :束縛して呼ぶ,   :bind_call ]) if (未束縛の操作.公開された操作は定義済みか? :bind_call, 偽値)
      資料('名前'           => [ :名前,           :name ],
           '元の名前'       => [ :元の名前,       :original_name ],
           '所属'           => [ :所属,           :owner ],
           '引数の一覧'     => [ :引数の一覧,     :parameters ],
    ...
    組(:ゴミ集めの操作の試験, 原型: 試験単位::試験事例) {
      資料('圧搾'                                       => [ :圧搾,                                       :compact ]) if (ゴミ集め.公開された操作は定義済みか? :compact, 偽値)
      資料('実行した回数'                               => [ :実行した回数,                               :count ],
           ...
           '頻繁に実施'                                 => [ :頻繁に実施,                                 :stress ],
           '頻繁に実施='                                => [ :頻繁に実施=,                                :stress= ])
      資料('圧搾した物への参照を検証する'               => [ :圧搾した物への参照を検証する,               :verify_compaction_references ]) if (ゴミ集め.公開された操作は定義済みか? :verify_internal_consistency, 偽値)
      資料('内部の一貫性を検証する'                     => [ :内部の一貫性を検証する,                     :verify_internal_consistency ],
