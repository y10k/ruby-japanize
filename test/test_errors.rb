  ...
  部(:試験) {
    組(:例外や失敗の操作の試験, 原型: 試験単位::試験事例) {
      [ [ '配置の失敗', [],
        ...
        [ '凍結済みのため失敗', [],
          [ [ :受信先, :receiver, 偽値, (凍結済みのため失敗.公開された操作は定義済みか? :receiver, 偽値) ]
          ]
        ],
        [ 'システムコールの失敗', [ '試験の伝文', 2 ],
          [ [ :失敗の種類を表す番号, :errno ] ]
        ...
        for 日本語の別名, 元の名前, 何故か操作が一致しない, 試験の対象か in 日本語の操作名の一覧
          if (試験の対象か.無値か? || 試験の対象か) then
            例外の型 = 評価(例外の名前)
    ...
    組(:システムコールの失敗の操作の試験, 原型: 試験単位::試験事例) {
      試験('失敗の種類を表す番号が親の失敗で定義されないことを確認') {
        例外が発生することを確認(操作が無いため失敗) {
          システムコールの失敗::失敗の種類を表す番号
        }
      }

      for 名前 in Errno.定数の一覧.grep(/^E/)
        資料(名前 => Errno.定数を取得(名前))
      end
      試験('失敗の種類を表す番号が子の失敗で定義されることを確認') {|資料|
        あるシステムコールの失敗 = 資料
        一種であることを確認(整数, あるシステムコールの失敗::失敗の種類を表す番号)
      }
    }
  }
}
