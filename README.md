...
#### バッククォート(`)記法によるメソッド(操作)の定義

バッククォート(`` ` ``)内で`定義`を使うことでメソッド(操作)を定義でき
ます。このバッククォート(`` ` ``)記法はRubyの`def`文によるメソッド(操作)
の定義に相当します。Rubyの`def`文は`end`で終了する必要がありますが、バッ
ククォート(`` ` ``)記法はバッククォート(`` ` ``)で終了が明示されるので
...
バッククォート(`` ` ``)記法によるメソッド(操作)の定義ではRuby 2.7で導
...
バッククォート(`` ` ``)記法によるメソッド(操作)の定義では末尾の引数を
